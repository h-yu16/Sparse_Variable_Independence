...
def main(args, round, logger):
    ...
    if args.reweighting == "DWR":
        ...
        W = SRDO(X_train, p_s, decorrelation_type=args.decorrelation_type, max_iter=args.iters_balance, random_state=args.seed)
    else:
        W = np.ones((n, 1))/n
    ...
    if args.paradigm == "regr":
        ...
        model = model_func(X_train, Y_train, W, **vars(args))
        beta_hat = model.coef_.reshape(-1)
        results["beta_hat"] = beta_hat
        logger.info("Model beta: %s" % pretty(beta_hat))
        logger.info("True beta: %s" % pretty(beta))         
        for metric in args.metrics:
            metric_func = get_metric_class(metric)
            results["%s_s" % metric] = metric_func(beta_hat[:p_s], beta_s)
            results["%s_v" % metric] = metric_func(beta_hat[p_s:], beta_v)
            results["%s_all" % metric] = metric_func(beta_hat, beta)
            for covariates in ["s", "v", "all"]:
                logger.info("%s for %s: %.3f" % (metric, covariates, results["%s_%s"%(metric, covariates)]))
